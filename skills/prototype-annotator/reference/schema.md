...
对应实现是 ui-annotator 的 `src/core/schema.js`。
...
> `glossary` 在**配置文件（导出/导入）里统一是数组**。运行时内部会转为 `{term: definition}` 映射以提高查找效率，这是工具实现细节。外部只需关心数组格式，`applyAiResult` 会处理格式转换。
...
- `strategies` — 有序的定位线索，回放时全部参与加权投票。9 种 `kind` 与权重见 [locating.md](locating.md)。`value` 形态随 `kind` 而定（字符串或数组），`extra` 放策略私有信息，如 `testId` 的属性名。**AI 输出的 `ref`（如 `e12`）由工具在 `applyAiResult` 阶段自动还原为此处的 `strategies`；写标注时只引用 ref，`target` 字段由工具填充。**
...
- 元素定位策略与降级机制 → [locating.md](locating.md)
- AI 输出契约与撰写规范 → [SKILL.md](../SKILL.md)
- 实现 → ui-annotator 的 `src/core/schema.js`、`src/core/locator.js`
